...
def test_check_method_show_all_rows(create_connection):
    ...
    with MyDatabaseContextManager(create_connection) as connector:
        ...
        print(result_db)
        expected_result = [(1, 'for_test_only@gmail.com', 'Stefan Muller', 'Star Wars', '2021-01-08'),
               (2, 'test@gmail.com', 'Martin Schmidt', 'Sherlock Holmes', '2024-05-28'),
               (3, 'test2@onet.pl', 'Jesus Navas', 'Rainmaker', '2020-03-16'),
               (4, 'for_test_only@gmail.com', 'Kylian Mbappe', 'Potop', '2023-02-25')
               ]
...
def test_show_reminder(create_connection_for_reminder):
    reminder_days = float(getenv("REMINDER"))
    with MyDatabaseContextManager(create_connection_for_reminder) as connector:
